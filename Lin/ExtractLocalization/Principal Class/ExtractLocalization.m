...
- (void)createMenuExtractLocalization {
    ...
    if (editMenu && ![editMenu.submenu itemWithTitle:@"MMString"]) {
        NSMenuItem *translateMenuItem = [[NSMenuItem alloc] initWithTitle:@"MMString" action:NULL keyEquivalent:@""];
        translateMenuItem.submenu = [[NSMenu alloc] init];
        [editMenu.submenu insertItem:translateMenuItem atIndex:0];
        ...
        [[translateMenuItem submenu]addItem:extractLocalizationStringMenu];
        [[translateMenuItem submenu]addItem:deleteLocalizationMenu];
    }
            ...
            if (keyStr.length > 0) {
                [EditorLocalizable deleteKey:keyStr atPath:self.defaultLocalizableFilePath andComplete:^(BOOL success) {
                    if (success) {
                        ...
                        [LinLogger info:@"delete key[%@] fail", keyStr];
                    }
                }];
            }
